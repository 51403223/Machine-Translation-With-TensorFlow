...
class EncoderBasic:
    ...
    def encode(self, batch_of_sentences):
        ...
        _, _, hidden_states = tf.while_loop(cond, body, [0, cell_state, hidden_states], swap_memory=True)
...
class GlobalAttentionDecoder:
    def __init__(self, lstm_cell, vocab_size, embeddings, attention_input_size):
        ...
        self.weight_score = tf.Variable(
            tf.random_uniform(shape=[lstm_cell.hidden_size, vocab_size], minval=-0.1, maxval=0.1)
        )
        ...
        self.bias_attention = tf.Variable(tf.zeros(shape=[lstm_cell.batch_size, lstm_cell.hidden_size]))
