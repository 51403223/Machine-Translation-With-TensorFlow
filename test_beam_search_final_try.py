...
def test_model(model_path, src_file_name, tgt_file_name, beam_width=3):
    # model_path='checkpoint_v1/model-11'; src_file_name='tst2012.vi'; tgt_file_name='tst2012.en'; beam_width=3
    ...
    with infer_graph.as_default():
        ...
        def loop_fn(time, cell_output, cell_state, log_probs, beam_finished):
            ...
            if cell_output is None:  # time = 0
                ...
                next_input = tuple(tf.nn.embedding_lookup(embedding_tgt, [sos_vocab_id] * batch_size) for _ in range(beam_width))
                predicted_ids = tuple(tf.constant(0) for _ in range(beam_width))  # https://github.com/hanxiao/hanxiao.github.io/issues/8
                ...
                for i in range(beam_width):
                    ...
                    values, indices = tf.nn.top_k(log_prob, beam_width, sorted=False)  # [batch, beam], [batch, beam]
                ...
                concat_vlist = tf.concat(tf.unstack(values_list, axis=0), axis=-1)  # [batch_size, beam_width*beam_width]
                concat_ilist = tf.concat(tf.unstack(indices_list, axis=0), axis=-1)
                top_values, index_in_vlist = tf.nn.top_k(concat_vlist, beam_width, sorted=False)  # [batch_size, beam_width]
                def get_word_ids(ids, ilist):
                    """
                    get values according to ids in ilist
                    ...
                    for i in range(batch_size):
                        current_ids = ids[i]
                        current_list = ilist[i]
                        word_ids.append(tf.map_fn(
                ...
                predicted_ids = tf.convert_to_tensor(get_word_ids(index_in_vlist, concat_ilist),
                                                       dtype=tf.int32)  # [batch_size, beam_width]
                new_beam_finished = tf.logical_or(tf.equal(word_ids_to_add, eos_vocab_id), beam_finished)
                ...
                elements_finished = tf.logical_or(elements_finished,
                                                  tf.equal(predict, eos_vocab_id))  # or finish by generated <eos>
        ...
        outputs_ta, final_state, _ = raw_rnn_for_beam_search(attention_cell, loop_fn)
